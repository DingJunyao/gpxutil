...
class RoutePointMongoSyncEntity(Document):
    ...
    def from_route_point_instance(route_point_instance: RoutePoint):
        return RoutePointMongoSyncEntity(
    ...
    def set_area(self):
        ...
        point_entity.set_area(area_gdf_list=GDFListHandler().list, area_code_conn=AreaCodeConnectHandler().get_connection(), force=False)
    ...
    def transform_coordinate_from_parent(self):
        point_entity = self.to_route_point_instance()
        parent_route = RouteMongoSyncEntity.get(self.route_id).run()
        point_entity.transform_coordinate(parent_route.coordinate_type, parent_route.transformed_coordinate_type, force=True)
        self.longitude_transformed = point_entity.longitude_transformed
        self.latitude_transformed = point_entity.latitude_transformed

class RouteMongoSyncEntity(Document):
    ...
    _temp_points: list[RoutePointMongoSyncEntity] = []
    ...
    def from_route_instance(
        ...
        route_points = [RoutePointMongoSyncEntity.from_route_point_instance(point) for point in route.points]
        return cls(
    ...
    def to_route_instance(self):
        points = RoutePointMongoSyncEntity.find(RoutePointMongoSyncEntity.route_id == self.id).run()
        return Route(
